...
##################################### INPUTS #################################################################
input_data = data_import("input_data.xlsx")
parameter_index = input_data["EMS"].index.tolist()
timeperiods = input_data["TA"].index.tolist()
reqs = input_data["MA"].loc[:,"REQ_1",]
parameter_profiles = input_data["EMS"]
parameter_profiles.sample(1, axis=1)

EMS_table = None
TA_table = input_data["TA"]
MA_table = input_data["MA"]
prices_table = input_data["Prices"]
                                     ...
                                     html.Div(className="row",style={"width":"80%", "margin-left":"12%"},
                                      ...
                                      html.Div(className="col",
                                            ...
                                            dcc.RadioItems(
                                                ...
                                                options=[{'label': dem, 'value': dem} for dem in input_data["Demand"].columns],
                                                value="dem_01",
                                      ...
                                      html.Div(className="col",#style={"margin-left":70,"margin-right":10},
                                            ...
                                            dcc.RadioItems(
                                                ...
                                                options=[{'label': gen, 'value': gen} for gen in input_data["Generation"].columns],
                                                value="gen_01",
                                                labelStyle={'display': 'inline-block', "margin":5}
                                                    )
                                                    ]),
...
#---------------------------------------------------checkboxes_profile-----------------------------------------------
@app.callback(Output('profile-graph', 'figure'),
...
def profile_switch(demand_profile, generation_profile):
    ...
    for profile in input_data["Generation"].columns:
        barplot = go.Bar(
            x=input_data["Generation"].index,
            y=input_data["Generation"].loc[:,generation_profile],
            name='Generation'
            )
        demand_traces[demand_profile]=barplot

    for profile in input_data["Demand"].columns:
        lineplot = go.Scatter(
            x=input_data["Demand"].index,
            y=input_data["Demand"].loc[:,demand_profile],
            mode='lines+markers',
            marker={
                'size': 15,
...
#----------------------------------------------------------any_clicks------------------------------------------
@app.callback(Output('any-clicks', 'children'),
...
#-----------------------------------------------------toggle_layout-----------------------------------------------------
@app.callback(Output('toggle-window', 'children'),
...
#----------------------------------------------------------active-ems------------------------------------------
@app.callback(Output('active-ems', 'children'),
              [Input('ems-slider', 'value'),
              ],
              )
...
#----------------------------------------------------------active_scenarios------------------------------------------
@app.callback(Output('active-sc', 'children'),
              [Input('sc-slider', 'value'),])
...
#----------------------------------------------------------run_name------------------------------------------
@app.callback(Output('run-name-output', 'children'),
...
#----------------------------------------------------------last_clicked------------------------------------------
@app.callback(Output('last-clicked', 'children'),
              [
...
def last_click_display(ta_stamp,ma_stamp,ems_stamp,prices_stamp,ta_click,
    	               ma_click, ems_click, prices_click,
                       ):
    last_clicked = ""
...
#----------------------------------------------------------selected-sc------------------------------------------
@app.callback(Output('selected-sc', 'children'),
              [Input('sc-tabs', 'value'),]
              )
...
def table_output(active_ems, active_sc, last_clicked, ta_click, ma_click,
                ems_click, prices_click, tabchange, selected_sc):
    ...
    if selected_sc in global_agent_clicked_dict.keys():
        print ("selected_sc {} is in global_agent_clicked_dict.keys \n".format(selected_sc))
        #pass
    else:
    ...
    return table
#----------------------------------------------------------dump_table_data-----------------------------------------------------
@app.callback(Output('p-bucket', 'children'),
              [Input('editable-table', 'dataframe'),]
              )
def dump_table_data(dataframe):
    return  json.dumps(dataframe)
#----------------------------------------------------------update_sets-----------------------------------------------------
@app.callback(Output('updated-parameter-set', 'children'),
              [
               Input('active-sc', 'children'),
               Input('last-clicked', 'children'),
...
def update_sets_after_user_input(active_sc, last_clicked, any_clicks, selected_sc, changed_table_data):
    global global_parameter_dict
    global global_timeseries_dict
...
def flags_2_parameter_set(selected_sc, last_clicked, run, active_sc, flex_split_flag, market_flags, neg_init,
                          ta_strategy, ma_strategy, optimization_mode,internal_balance, flex_options):
    global global_parameter_dict
    try:
        selected_sc = json.loads(selected_sc)
        active_sc = json.loads(active_sc)
...
def start_run(run_click, active_scenarios, active_ems):
    ...
    global global_environments_dict
    global global_output_files_dict
    ...
    for scenario in active_scenarios:
        ma_dfx = dict()
        ta_dfx = dict()
        for item, request in zip(global_timeseries_dict[scenario]["ma"].items(),global_req_names):
            df = pd.Series(item[1])
            df = pd.to_numeric(df)
            df.index.name = "time"
            ma_dfx[request] = df

        for item, request in zip(global_timeseries_dict[scenario]["ta"].items(),global_req_names):
            df = pd.Series(item[1])
            df = pd.to_numeric(df)
            df.index.name = "time"
            ta_dfx[request] = df

        multi_ix = pd.MultiIndex.from_product([global_timeperiods, active_ems], names=['time', 'ems'])
        ...
        for ems in active_ems:
            if global_parameter_dict[scenario]["ems"][ems]['Generation-Profile'] == 1:
                generation_curve = input_data["Generation"].loc[:,"gen_01"]
            elif global_parameter_dict[scenario]["ems"][ems]['Generation-Profile'] == 2:
                generation_curve = input_data["Generation"].loc[:,"gen_02"]
            elif global_parameter_dict[scenario]["ems"][ems]['Generation-Profile'] == 3:
                generation_curve = input_data["Generation"].loc[:,"gen_03"]

            if global_parameter_dict[scenario]["ems"][ems]['Demand-Profile'] == 1:
                demand_curve = input_data["Demand"].loc[:,"dem_01"]
            elif global_parameter_dict[scenario]["ems"][ems]['Demand-Profile'] == 2:
                demand_curve = input_data["Demand"].loc[:,"dem_02"]
            elif global_parameter_dict[scenario]["ems"][ems]['Demand-Profile'] == 3:
                demand_curve = input_data["Demand"].loc[:,"dem_03"]

            ems_df = pd.DataFrame()
            ems_df["dem"] = demand_curve * float(global_parameter_dict[scenario]["ems"][ems]['Demand-Total'])
            ems_df["gen"]  = generation_curve * float(global_parameter_dict[scenario]["ems"][ems]['Generation-Total'])
            ...
            for time in global_timeperiods:
                ...
                multi_ix_df.at[(time,ems),"mp"] = global_timeseries_dict[scenario]["prices"][("MP_"+ems)][str(time)]
                multi_ix_df.at[(time,ems),"fit"] = global_timeseries_dict[scenario]["prices"][("FIT_"+ems)][str(time)]
        ...
        env = Environment(run_name="Run_01",
...
def upper_left_inside_update(run_button_clicked, offer_slider_value, tab_name, scenario_name,checkbox,run_name_update):
        ...
        if sc == scenario_name:
            ...
            for offer in enumerate(ems_plot_data.keys()):
                if offer_slider_value == offer[0]:
                    act_offer = offer[1]
                    ems_plot_data = global_timeseries_dict[scenario_name]["ems"][offer[1]]
                else:
                    pass

    if tab_name == "Timeseries":

        blue_colors = ["lightskyblue","deepskyblue","cornflowerblue","dodgerblue","steelblue","darkslateblue","royalblue","blue","midnightblue"]
        yellow_colors = ["rgb(249, 200, 6)","moccasin","peachpuff","khaki","yellow","rgb(204,204,0)","rgb(153,153,0)","rgb(102,102,0)","rgb(128,128,0)"]
        purple_colors = ["plum","fuchsia","hotpink","deeppink","mediumorchid","mediumpurple","mediumvioletred","blueviolet","indigo"]
        light_green_colors = ["lawngreen","limegreen","springgreen","mediumseagreen","seagreen","darkgreen","olivedrab","darkolivegreen","darkgreen"]
        dark_green_colors = ["olivedrab","darkgreen","darkolivegreen","darkgreen","lawngreen","limegreen","springgreen","mediumseagreen","seagreen"]
        red_colors = ["indianred","darkred","crimson","darkred","darksalmon","red","firebrick","salmon","lightsalmon"]
        brown_colors = ["burlywood","goldenrod","chocolate","tan","rosybrown","saddlebrown","sandybrown","peru","brown"]
        grey_colors_1 = ["gainsboro","darkslategray","black","lightgray","gray","lightslategray","silver","dimgray","lightslategray","gainsboro","darkslategray","black"]
        grey_colors_2 = ["gainsboro","darkslategray","black","silver","dimgray","lightslategray","lightgray","gray","lightslategray"]
        cyan_colors = ["cyan","aquamarine","darkcyan","aqua","turquoise","teal","darkturquoise","paleturquoise","lightseagreen"]
        orange_colors = ["coral","orange","darkorange","tomato","firebrick","firebrick","salmon","orange","darkred"]
        ...
        for ems in ems_plot_data.index.get_level_values('ems').unique():
            ...
            barplot = go.Bar(
                x=global_timeperiods,
                y=[ems_plot_data.loc[(t,ems),"_Generation"] for t in global_timeperiods],
                textposition = 'auto',
                hoverinfo = "y+text",
                hoverlabel = dict(
                    font = dict(
        ...
        for ems in ems_plot_data.index.get_level_values('ems').unique():
            barplot = go.Bar(
                x=global_timeperiods,
                y=[ems_plot_data.loc[(t,ems),"Net_Demand"] for t in global_timeperiods],
                textposition = 'auto',
                hoverinfo = "y+text",
                hoverlabel = dict(
                    font = dict(
        ...
        for ems in ems_plot_data.index.get_level_values('ems').unique():
            barplot = go.Bar(
                x=global_timeperiods,
                y=[ems_plot_data.loc[(t,ems),"Net_Generation"] for t in global_timeperiods],
                textposition = 'auto',
                hoverinfo = "y+text",
                hoverlabel = dict(
                    font = dict(
        ...
        for ems in ems_plot_data.index.get_level_values('ems').unique():
            barplot = go.Bar(
                x=global_timeperiods,
                y=[ems_plot_data.loc[(t,ems),"Request_POS"] for t in global_timeperiods],
                textposition = 'auto',
                hoverinfo = "y+text",
                hoverlabel = dict(
                    font = dict(
        ...
        for ems in ems_plot_data.index.get_level_values('ems').unique():
            barplot = go.Bar(
                x=global_timeperiods,
                y=[ems_plot_data.loc[(t,ems),"Request_NEG"] for t in global_timeperiods],
                textposition = 'auto',
                hoverinfo = "y+text",
                hoverlabel = dict(
                    font = dict(
        ...
        data = []
        data += [demand_traces[ems] for ems in ems_plot_data.index.get_level_values('ems').unique()]
        data += [generation_traces[ems] for ems in ems_plot_data.index.get_level_values('ems').unique()]
        data += [buy_traces[ems] for ems in ems_plot_data.index.get_level_values('ems').unique()]
        data += [sell_traces[ems] for ems in ems_plot_data.index.get_level_values('ems').unique()]
        data += [flex_pos_req_trace[ems] for ems in ems_plot_data.index.get_level_values('ems').unique()]
        data += [flex_neg_req_trace[ems] for ems in ems_plot_data.index.get_level_values('ems').unique()]
...
def lower_left_inside_update(run_button_clicked, offer_slider_value, tab_name, scenario_name, checkbox):
    ems_plot_data = pd.DataFrame()
            ...
            for offer in enumerate(ems_plot_data.keys()):
                if offer_slider_value == offer[0]:
                    ...
                    if offer_slider_value != 0:
                        ...
                        request_timeseries = global_timeseries_dict[scenario_name]["ma"][global_req_names[request_nr]]
                else:
                    pass

    grey_colors = ["lightgray","gray","	lightslategray","silver","dimgray","lightslategray","gainsboro","darkslategray","black"]
    cyan_colors = ["aquamarine","darkcyan","aqua","turquoise","teal","darkturquoise","paleturquoise","lightseagreen", "cyan"]
    orange_colors = ["coral","orange","darkorange","tomato","firebrick","firebrick","salmon","orange","darkred"]
    green_colors = ["lawngreen","limegreen","springgreen","mediumseagreen","seagreen","darkgreen","olivedrab","darkolivegreen","darkgreen"]
    red_colors = ["lightsalmon","indianred","darkred","salmon","crimson","darkred","darksalmon","red","firebrick"]
    dark_green_colors = ["olivedrab","darkgreen","springgreen","darkgreen","lawngreen","limegreen","springgreen","mediumseagreen","seagreen"]
    brown_colors = ["burlywood","goldenrod","chocolate","tan","rosybrown","saddlebrown","sandybrown","peru","brown"]
    purple_colors = ["mediumvioletred","blueviolet","indigo","plum","fuchsia","hotpink","deeppink","mediumorchid","mediumpurple"]
    ...
    for ems in ems_plot_data.index.get_level_values('ems').unique():
        barplot = go.Bar(
            x=global_timeperiods,
            y=[ems_plot_data.loc[(t,ems),"Battery_Charged"] for t in global_timeperiods],
    ...
    for ems in ems_plot_data.index.get_level_values('ems').unique():
        barplot = go.Bar(
            x=global_timeperiods,
            y=[ems_plot_data.loc[(t,ems),"Battery_Discharged"] for t in global_timeperiods],
    ...
    for ems in ems_plot_data.index.get_level_values('ems').unique():
        markerplot = go.Scatter(
            x=global_timeperiods,
            y=[ems_plot_data.loc[(t,ems),"Battery_Switched"] for t in global_timeperiods],
            name=("{}_b_on".format(ems)),
            mode = 'markers',
            marker=go.Marker(color=batt_switched_on_colors[ems], size=12, line=dict(color = "black")),
            )
    ...
    for ems in ems_plot_data.index.get_level_values('ems').unique():
        barplot = go.Bar(
            x=global_timeperiods,
            y=[ems_plot_data.loc[(t,ems),"Battery_Flexibility_POS"] for t in global_timeperiods],
            textposition = 'auto',
            hoverinfo = "y+text",
            hoverlabel = dict(
                font = dict(
    ...
    for ems in ems_plot_data.index.get_level_values('ems').unique():
        barplot = go.Bar(
            x=global_timeperiods,
            y=[ems_plot_data.loc[(t,ems),"Battery_Flexibility_NEG"] for t in global_timeperiods],
            textposition = 'auto',
            hoverinfo = "y+text",
            hoverlabel = dict(
                font = dict(
    ...
    if "Capacity" in checkbox:
        data += [cap_traces[ems] for ems in ems_plot_data.index.get_level_values('ems').unique()]
        data += [charge_traces[ems] for ems in ems_plot_data.index.get_level_values('ems').unique()]
        data += [discharge_traces[ems] for ems in ems_plot_data.index.get_level_values('ems').unique()]

    if "Starts" in checkbox:
        data += [batt_switched_on_traces[ems] for ems in ems_plot_data.index.get_level_values('ems').unique()]

    if "Flexibilities" in checkbox:
        data += [batt_flex_charge_traces[ems] for ems in ems_plot_data.index.get_level_values('ems').unique()]
        data += [batt_flex_discharge_traces[ems] for ems in ems_plot_data.index.get_level_values('ems').unique()]
...
def price_plots_update(run_button_clicked, offer_slider_value, tab_name, scenario_name, checkbox):
    ems_plot_data = pd.DataFrame()
    request_timeseries = pd.Series(0, index = global_timeperiods)
    try:
        ems_plot_data = global_timeseries_dict[scenario_name]["ems"]
    ...
    data = []
    data += [mp_prices_traces[ems] for ems in ems_plot_data.index.get_level_values('ems').unique()]
    data += [fit_prices_traces[ems] for ems in ems_plot_data.index.get_level_values('ems').unique()]
