...
from comopt.model.utils import root_divided_by_2, linear, cos_root_divided_by_2, gauss_1, gauss_2, uniform_1, no_noise
...
def get_MA_bid_from_sample(policy_function: Callable):
    ...
    def policy_function_wrapper(rounds_total: int,
                                rounds_left: int,
                                ma_parameter: dict) -> dict:

        ma = policy_function(rounds_total=rounds_total, rounds_left=rounds_left, ma_parameter=ma_parameter)
        ma["Markup"] = (ma["Markup"] + ma["Noise"](rounds_total=rounds_total, rounds_left=rounds_left, mean=ma["Markup"])) * ma["Concession"](rounds_total=rounds_total, rounds_left=rounds_left)
        ...
        return {"Bid":ma["Bid"], "Reservation price":ma["Reservation price"], "Markup":ma["Markup"]}
...
def never_buy(rounds_total: int,
              rounds_left: int,
    ...
    return {"Reservation price":reservation_price, "Markup":markup , "Shape":concession_curve}

@get_MA_bid_from_sample
# Selling with a reservation price of +inf
def buy_at_any_cost(rounds_total: int,
                    rounds_left: int,
                    reservation_price=float("inf"),
                    markup: Union[int,float] = 1,
                    concession_curve: Union[root_divided_by_2, linear] = linear,
...
def buy_with_deterministic_prices(rounds_total: int,
                                  rounds_left: int,
                                  ma_parameter: dict,
                                  ) -> dict:
...
def buy_with_stochastic_prices(rounds_total: int,
                               rounds_left: int,
                               ma_parameter: dict) -> dict:

    # Add additional logic here:
    # e.g if rounds_left == 5, then reservation_price == 10
    return {"Bid": 0, "Reservation price":ma_parameter["Reservation price"], "Markup":ma_parameter["Markup"],
            "Concession":ma_parameter["Concession"], "Noise":ma_parameter["Noise"]}
