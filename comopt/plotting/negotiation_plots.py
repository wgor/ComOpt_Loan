...
def plot_negotiation_data(
    input_data: dict,
    negotiation_data_df: DataFrame,
    q_tables: dict = None,
    action_tables: dict = None,
):
    ...
    gs = gridspec.GridSpec(
        12,
        4,
        height_ratios=[2, 1.5, 1.5, 1, 1, 1, 1, 1, 1, 1, 1, 0.75],
    ...
    clearing_price_mean = (
        df["Clearing price"].groupby("Round").sum()
        / df["Cleared"].groupby("Round").sum()
    )
    ta_bids_mean = (
        df["TA bid"].groupby("Round").sum() / df["TA bid"].groupby("Round").count()
    )
    ma_bids_mean = (
        df["MA bid"].groupby("Round").sum() / df["MA bid"].groupby("Round").count()
    )
    ta_mean_profit = [
        df.groupby(["Round"]).get_group(round)["TA profit"].sum()
        / df.groupby(["Round"]).get_group(round)["Cleared"].sum()
        if df.groupby(["Round"]).get_group(round)["Cleared"].sum() > 0
        else 0
        for round in rounds_index
    ]
    ma_mean_profit = [
        df.groupby(["Round"]).get_group(round)["MA profit"].sum()
        / df.groupby(["Round"]).get_group(round)["Cleared"].sum()
        if df.groupby(["Round"]).get_group(round)["Cleared"].sum() > 0
        else 0
        for round in rounds_index
    ...
    ratio = (
        df[df["Cleared"] > 0]["Cleared"].sum() / len(total_number_of_clearings) * 100
    )
    ...
    ax0.set_title(
        "Number of Negotiatons: {}, Rounds each: {}".format(
            len(datetime_index), len(rounds_index)
        ),
    ...
    ax1.set_ylim(
        df.groupby(["Round"]).get_group(1)["TA reservation price"][0],
    ...
    ax2.set_title(
        "Averaged bid and clearing price values per round", fontsize=fontsize_titles
    )

    # ax3 Average Profits per round + number of clearings per round
    ax31 = plt.subplot(gs[3, 0:2])
    ax31.set_title(
        "Clearings and AVG bids per round and agent", fontsize=fontsize_titles
    )
    ax32 = ax31.twinx()
    ...
    try:
        legend_elements_ax27 = [
            Line2D(
                ...
                [0],
                linestyle="",
                label="MA policy: {}".format(
                    input_data["MA prognosis policy"].__name__
                ),
            ),
            Line2D(
                [0],
                [0],
                linestyle="",
                label="TA policy: {}".format(
                    input_data["TA prognosis policy"].__name__
                ),
            ),
            Line2D(
                [0],
                [0],
                linestyle="",
                label="Exploration: {}".format(
                    input_data["Q parameter prognosis"]["Exploration function"].__name__
                ),
            ),
            Line2D(
                [0],
                ...
                linestyle="",
                label="Action: {}".format(
                    input_data["Q parameter prognosis"]["Action function"].__name__
                ),
            ),
            Line2D(
                [0],
                [0],
                linestyle="",
                label="Alpha: {}".format(input_data["Q parameter prognosis"]["Alpha"]),
            ),
            Line2D(
                [0],
                [0],
                linestyle="",
                label="Gamma: {}".format(input_data["Q parameter prognosis"]["Gamma"]),
            ),
            Line2D(
                [0],
                [0],
                ...
                label="Epsilon: {}".format(
                    input_data["Q parameter prognosis"]["Epsilon"]
                ),
            ),
    ...
    try:
        ...
        for table, ax in zip(list(q_tables.items()), heatmaps):
            ...
            ax.set_title(
                "Environment runtime: {} %".format(
                    round(100 * table[0][0] / (len(datetime_index))), fontsize=12
                )
            )
        ...
        for ax in [ax0, ax1]:
            for datetime in datetime_index:
                ax.plot(
                    rounds_index,
                    "MA bid",
                    data=df.groupby(["Datetime"]).get_group(datetime),
                    marker="X",
    ...
    legend_elements_ax1 = [
        Line2D([0], [0], color="dodgerblue", linestyle="dashed", label="MA res"),
        ...
        Line2D(
            [0], [0], marker="o", markerfacecolor="red", markersize=16, label="Clearing"
        ),
    ]
    ...
    # ax3 Average Profits
    ax31.bar(
        x=rounds_index - bar_width,
        height=ta_mean_profit,
        width=bar_width,
        color="white",
    ...
    ax32.legend(
        bbox_to_anchor=(0.45, 1), fancybox=True, ncol=3, fontsize=14, framealpha=0.8
    )

    # ax4: Clearing price distributions
    clearing_boxes = [
        df.groupby(["Round"]).get_group(round)["Clearing price"].dropna()
        for round in rounds_index
    ]
    clearing_boxes = [list(box.values.flatten()) for box in clearing_boxes]
    bp = ax4.boxplot(
        [x for x in clearing_boxes],
        positions=rounds_index,
        patch_artist=True,
        widths=0.25,
    )
    ...
    ax5.plot(
        datetime_index,
        df.groupby(["Datetime"])["MA profit"].sum().cumsum(),
        marker="+",
        markerfacecolor="black",
        color="dodgerblue",
        label="MA cum profit",
    ...
    ax5.plot(
        datetime_index,
        df.groupby(["Datetime"])["TA profit"].sum().cumsum(),
        color="forestgreen",
        linestyle="dashed",
        label="TA cum profit",
    )
    ...
    legend_elements_ax5 = [
        Line2D(
            ...
            color="dodgerblue",
            linestyle="-",
            label="MA Profits: {}".format(round(total_profit_MA)),
        ),
        Line2D(
            [0],
            [0],
            color="forestgreen",
            linestyle="dashed",
            label="TA Profits: {}".format(round(total_profit_TA)),
        ),
        Line2D(
            [0],
            [0],
            color="",
            linestyle="",
            label="TA profits %: {}".format(
                round(total_profit_TA / (total_profit_MA + total_profit_TA) * 100, 2)
            ),
        ),
        Line2D(
            [0],
            ...
            color="",
            linestyle="",
            label="MA profits %: {}".format(
                round(total_profit_MA / (total_profit_MA + total_profit_TA) * 100, 2)
            ),
        ),
    ]
    ...
    # ax6: Boxplot TA bids
    ta_bid_boxes = [
        df.groupby(["Round"]).get_group(round)["TA bid"].dropna()
        for round in rounds_index
    ]
    ta_bid_boxes = [list(box.values.flatten()) for box in ta_bid_boxes]
    bp = ax6.boxplot(
        [x for x in ta_bid_boxes],
        positions=rounds_index,
        patch_artist=True,
        widths=0.25,
    )
    ...
    ax7.plot(
        datetime_index,
        cleared_rounds_cumulated,
        marker="",
        markerfacecolor="black",
        linestyle="-",
        color="Black",
        label="Cleared Rounds",
    )

    ax7.plot(
        datetime_index,
        total_number_of_clearings,
        marker="",
        markerfacecolor="black",
        linestyle="dashed",
        color="red",
    ...
    legend_elements_ax7 = [
        Line2D(
            ...
            color="red",
            linestyle="dashed",
            label="Total number of Clearings: {}".format(
                len(total_number_of_clearings)
            ),
        ),
        Line2D(
            [0],
            ...
            color="black",
            linestyle="-",
            label="Cleared rounds: {}".format(df[df["Cleared"] > 0]["Cleared"].sum()),
        ),
        Line2D(
            [0], [0], linestyle="None", label="Cleared: {} %:".format(round(ratio, 2))
        ),
    ]
    ...
    # ax8: Boxplot MA bids
    ma_bid_boxes = [
        df.groupby(["Round"]).get_group(round)["MA bid"].dropna()
        for round in rounds_index
    ]
    ma_bid_boxes = [list(box.values.flatten()) for box in ma_bid_boxes]
    bp = ax8.boxplot(
        [x for x in ma_bid_boxes],
        positions=rounds_index,
        patch_artist=True,
        widths=0.25,
    )
    ...
    ax9.scatter(
        x=x, y=y, marker="o", color="forestgreen", linewidths=4, label="Profit(Markup)"
    )
    ...
    ax10.scatter(
        x=x, y=y, marker="X", color="blue", linewidths=4, label="Profit(Markup)"
    )
