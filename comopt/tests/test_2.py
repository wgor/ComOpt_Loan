...
deviation_prices = [2**x for x in range(4,4+len(imbalances_test_profile_1_day.index))]
deviation_prices = Series(index=imbalances_test_profile_1_day.index, data=deviation_prices)
#deviation_prices = imbalance_market_costs_normalized
...
input_data = {
    ...
    "Device constraints":  [
        ...
        [   #>>>>>> EMS 1 <<<<<#
            ...
            # 2) Generation
            follow_generated_production_profile(start=start, end=end, resolution=resolution, max_capacity=10, dispatch_factor=dispatch_factor_solar, profile=solar_test_profile_1_day),
