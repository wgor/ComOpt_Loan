...
from pandas import DataFrame, MultiIndex, IndexSlice, set_option, plotting, concat, date_range, option_context, to_numeric
...
end = datetime(year=2018, month=6, day=2, hour=0)
...
for x in deviation_prices.index:
    if deviation_prices[x] > 0.20:
        deviation_prices[x] = deviation_prices[x] + (uniform(0,100)*randint(0,1))

undispatched_load = load_test_profile_1_day
dispatched_load = enlopy.generate.gen_demand_response(load_test_profile_1_day,
...
for idx in flexible_load_profile.index:
    flexible_load_profile.loc[idx,"derivative min"] = min(dispatched_load[idx],undispatched_load[idx])
    flexible_load_profile.loc[idx,"derivative max"] = max(dispatched_load[idx],undispatched_load[idx])
...
net_demand_costs[net_demand_costs>0] = net_demand_costs[net_demand_costs>0]*purchase_price
net_demand_costs[net_demand_costs<0] = net_demand_costs[net_demand_costs<0]*feed_in_price
#------------OPTIONAL END-------------#
...
input_data = {
    ...
    "Device constraints":  [
        ...
        [   #>>>>>> EMS 1 <<<<<#
            ...
            dispatchable_load_profile_with_bounds(start=start, end=end, resolution=resolution, profile=flexible_load_profile),

            # 2) Generation
            follow_generated_production_profile(start=start, end=end, resolution=resolution,
                                                max_capacity=10, dispatch_factor=dispatch_factor_solar, profile=solar_test_profile_1_day),
    ...
    "MA prognosis parameter": {"Reservation price": 3,
                               "Markup": 1,
    ...
    "MA flexrequest parameter": {"Reservation price": 3,        # Placeholder variable
    ...
    "TA flexrequest parameter": {"Reservation price": 2,        # Placeholder variable
                                 "Markup factor": 1,
                                 "Markup": 1,                   # Placeholder variable
                                 "Concession": linear, # linear, root_divided_by_2, cos_root_divided_by_2, no_shape
...
ems.planned_power_per_device
env.trading_agent.cleared_prognosis_negotiations
env.trading_agent.cleared_flex_negotiations

ems.prognosed_power_per_device
...
def plot_ems_data(environment):
    ...
    ma_imbalances = env.market_agent.balancing_opportunities["Imbalance (in MW)"].apply(to_numeric, errors='ignore')
    ma_imbalance_market_prices = env.market_agent.balancing_opportunities["Price (in EUR/MWh)"].apply(to_numeric, errors='ignore')
    ma_imbalance_market_costs = env.market_agent.imbalance_market_costs.apply(to_numeric, errors='ignore')
    ma_bought_flexibility = env.market_agent.commitments["bought_flex"].apply(to_numeric, errors='ignore')
    ma_deviation_prices = Series(data=env.market_agent.deviation_prices_realised, index=index)
    ...
    ma_imbalance_market_cost_normalized = ma_imbalance_market_cost_normalized[start:end-env.max_horizon]
    ...
    load_flex_activated_plot.set_xlabel('Net Demand with and without Flex activation', fontsize=15)

    generation_flex_activated_plot.set_ylabel('Power in kW', fontsize=15)
    generation_flex_activated_plot.set_xlabel('Net Demand with and without Flex activation', fontsize=15)

    # EMS CONFIG: Net demand without flex
    net_demand_plot.set_ylabel('Power in kW', fontsize=15)
    net_demand_plot.set_xlabel('Net Demand with and without Flex activation', fontsize=15)

    # --------------------- MA PLOTS -------------------------#
    ma_imbalance_market_prices_plot.plot(index, ma_imbalance_market_prices, color="red", linewidth=3)
    ma_imbalances_plot.bar(x=index, height=ma_bought_flex, width=0.01, color="white", alpha=1, edgecolor='red', linewidth=5, linestyle="solid")
    ma_imbalances_plot.bar(x=index, height=ma_imbalances, width=0.01, facecolor="grey", alpha=0.3, edgecolor='black', linewidth=3, linestyle="solid")
    ma_imbalances_plot.plot(cleared_flex_negotiations, linewidth=0, linestyle="solid", marker="o")

    ma_imbalance_market_costs_plot.plot(index, ma_imbalance_market_cost_normalized, color="green", linewidth=3)
    ma_imbalance_market_costs_plot.fill_between(index, 0, ma_imbalance_market_cost_normalized, color="green", alpha=0.15)
    ma_deviation_prices_plot.bar(x=index, height=ma_deviation_prices, width=0.01, color="lightblue", alpha=0.8, edgecolor='black', linewidth=2, linestyle="solid")
    ma_deviation_prices_plot.bar(x=index, height=-ma_deviation_prices, width=0.01, color="blue", alpha=0.35, edgecolor='black', linewidth=2, linestyle="solid")
    # --------------------- EMS PLOTS -------------------------#
    ...
    if load_without_flex["derivative equals"].isnull().all():
        ...
        net_demand_plot.plot(index, flexible_load_lower_bound, color="blue", linestyle="dashed")
        net_demand_plot.plot(index, flexible_load_upper_bound, color="blue", linestyle="dashed")
        net_demand_plot.fill_between(index, flexible_load_lower_bound, flexible_load_upper_bound, color="lightblue", alpha=0.5)
        load_flex_activated_plot.plot(index, flexible_load_lower_bound, color="blue", linestyle="dashed")
        load_flex_activated_plot.plot(index, flexible_load_upper_bound, color="blue", linestyle="dashed")
        load_flex_activated_plot.fill_between(index, flexible_load_lower_bound, flexible_load_upper_bound, color="lightblue", alpha=0.5)
        load_flex_activated_plot.fill_between(index, flexible_load_lower_bound, realised_loads, color="red", alpha=0.5)
        load_flex_activated_plot.plot(index, realised_loads, color="black", linestyle="solid", linewidth=3, label="Realised Power")
    ...
    if generation_without_flex["derivative equals"].isnull().all():
        ...
        net_demand_plot.fill_between(index, 0, full_curtailable_generation, color="orange", alpha=0.5)
        generation_flex_activated_plot.plot(index, realised_generation, color="black", linestyle="solid", linewidth=3,label="Realised Power")
        generation_flex_activated_plot.plot(index, full_curtailable_generation, linestyle="dashed")
        generation_flex_activated_plot.fill_between(index, 0, full_curtailable_generation, color="orange", alpha=0.5)
        generation_flex_activated_plot.fill_between(index, realised_generation, full_curtailable_generation, color="red", alpha=0.5)
    ...
    net_demand_plot.plot(index, net_demand_without_flex, color="black", linewidth=3, linestyle="dashed", label="Net Demand")

    # EMS PLOT: net demand with flex
    net_demand_with_flex = realised_loads + realised_generation
    net_demand_plot.plot(index, net_demand_with_flex, color="black", linewidth=3, label="Net Demand + Flex")
    net_demand_plot.fill_between(index, net_demand_without_flex, net_demand_with_flex, color="red", alpha=0.5, label="Flexibilty")
    net_demand_plot.legend(loc='upper left', fancybox=True, ncol=6, fontsize=14, framealpha=0.8)


    for ax in [net_demand_plot,generation_flex_activated_plot,load_flex_activated_plot, ma_imbalance_market_prices_plot,ma_imbalances_plot, ma_imbalance_market_costs_plot,ma_deviation_prices_plot]:
