...
class TradingAgent(Agent):
    ...
    def create_flex_offer(
        self, flex_request: FlexRequest
    ) -> Tuple[FlexOffer, List[UdiEvent]]:
            ...
            for flex_split_method in flex_split_methods:
                ...
                planned_flex = initialize_series(
                    data=[
                        ...
                        for x in zip(
                            *[
                                udi_event.planned_flex.loc[
                                    udi_event.start : udi_event.end
                                    - udi_event.resolution
                                ].values
                                for udi_event in udi_events
                            ]
                        )
    ...
    def store_commitment(
        self, com: Union[Prognosis, FlexOrder], udi_events: List[UdiEvent]
    ):
    ...
    def step(self):
        ...
        for ems in self.ems_agents:
            ...
            self.environment.plan_board.store_message(
                timeperiod=self.environment.now, message=device_message, keys=[ems.name]
            )
            ...
            for event in udi_events:
                self.environment.plan_board.store_message(
                    timeperiod=self.environment.now, message=event, keys=[ems.name]
                )
        ...
        self.environment.plan_board.store_message(
            timeperiod=self.environment.now, message=prognosis, keys=["TA", "MA"]
        )
        ...
        self.environment.plan_board.store_message(
            timeperiod=self.environment.now, message=flex_request, keys=["TA", "MA"]
        )

        # print(flex_request.requested_flex)
        print(
            "TA: Received requested flex values in kWh:{}\n".format(
                flex_request.requested_flex
            )
        )
        print(
            "TA: Received requested power values in kWh:{}\n".format(
                flex_request.commitment.constants.values
            )
        )

        print(
            "---------------------FLEX UDI EVENTS--------------------------      TIME: {}".format(
                self.environment.now
            )
        )
        ...
        self.environment.plan_board.store_message(
            timeperiod=self.environment.now, message=flex_offer, keys=["TA", "MA"]
        )
        ...
        self.flexrequest_parameter[
            "Reservation price"
        ] = (
            flex_offer.costs
        )  # + self.environment.ems_agents[0].planned_flex_per_device.sum().sum() * 6

        print(
            "TA: Flex offer costs: {}".format(
                self.flexrequest_parameter["Reservation price"]
            )
        )
        print(
            "TA: Market agent reservation price {}".format(
                self.environment.market_agent.flexrequest_parameter["Reservation price"]
            )
        )
        ...
        print(
            "\nTA: Flex negotiation status: {}".format(
                flexrequest_decision_gate_1["Status"]
            )
        )
        print(
            "TA: Flex negotiation clearing price: {}\n".format(
                flexrequest_decision_gate_1["Clearing price"]
            )
        )
        print(
            "----------------------REALISED VALUES UPDATE--------------------------       TIME: {}".format(
                self.environment.now
            )
        )
