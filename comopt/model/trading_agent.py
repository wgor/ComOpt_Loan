...
from comopt.model.utils import initialize_df, initialize_index, initialize_series, start_negotiation, table_snapshots
...
class TradingAgent(Agent):
    ...
    def __init__(
        ...
        self.cleared_prognosis_negotiations = DataFrame(index=initialize_index(environment.start, environment.end, environment.resolution),
                                                        columns=["Cleared", "Clearing Price"])

        self.cleared_flex_negotiations = DataFrame(index=initialize_index(environment.start, environment.end, environment.resolution),
                                                        columns=["Cleared", "Clearing Price"])
        ...
        self.prognosis_q_table_df_1 = DataFrame(data=0,
                                                index=range(1,prognosis_rounds+1),
                                                columns=self.prognosis_q_parameter["Action function"](action=None,
                                                                                            markup=None,
                                                                                            show_actions=True).keys()
                                                )
        self.prognosis_q_table_df_2 = DataFrame(data=0,
                                                index=range(1,prognosis_rounds+1),
                                                columns=self.prognosis_q_parameter["Action function"](action=None,
                                                                                            markup=None,
                                                                                            show_actions=True).keys()
        ...
        self.flexrequest_q_table_df_1 = DataFrame(data=0,
                                                index=range(1,flexrequest_rounds+1),
                                                columns=self.flexrequest_q_parameter["Action function"](action=None,
                                                                                            markup=None,
                                                                                            show_actions=True).keys()
                                                  )
        self.flexrequest_q_table_df_2 = DataFrame(data=0,
                                                index=range(1,flexrequest_rounds+1),
                                                columns=self.flexrequest_q_parameter["Action function"](action=None,
                                                                                            markup=None,
                                                                                            show_actions=True).keys()
        ...
        self.store_table_steps = linspace(1, self.environment.total_steps, num=8, dtype="int", endpoint=True)
    ...
    def create_device_message(
        ...
        costs: float = None,) -> DeviceMessage:
    ...
    def create_prognosis(self, udi_events: List[UdiEvent]) -> Prognosis:
        ...
        for event in udi_events:
            prognosed_values = prognosed_values.add(event.commitment.constants[:], fill_value=0)
    ...
    def create_flex_offer(self, flex_request: FlexRequest) -> Tuple[FlexOffer, List[UdiEvent]]:
        ...
        if self.central_optimization is True:
            ...
            for flex_split_method in flex_split_methods:
                ...
                output = flex_split_method(self.ems_agents, flex_request, self.environment)
                targeted_power = output["target_power"]
                targeted_flex = output["target_flex"]
                ...
                for ems in self.ems_agents:
                    ...
                    for idx in targeted_flex.index:
                        ems.targeted_flex.loc[idx] = targeted_flex.loc[idx]
                ...
                # print([event.offered_power.values for event in udi_events])
                # print("PRINT")
                # print([event.planned_flex.loc[event.start:event.end-event.resolution].values for event in udi_events])
                # planned_flex_values = [event.planned_flex.loc[event.start:event.end-event.resolution].values for event in udi_events]
                # Calculate aggregated power values and total costs (private and bid)
                offered_values = initialize_series(
                    data=[
                        sum(x)
                        for x in zip(
                            *[
                ...
                # Added planned_flex
                planned_flex = initialize_series(
                    data=[
                        sum(x)
                        for x in zip(
                            *[
                             udi_event.planned_flex.loc[udi_event.start:udi_event.end-udi_event.resolution].values
                             for udi_event in udi_events
                            ]
                        )
                    ],
                ...
                private_costs = [udi_event.costs for udi_event in udi_events]
                reservation_costs = sum(private_costs)
                ...
                aggregated_udi_events.append(
                    UdiEvent(
                        ...
                        planned_flex=planned_flex,
                        costs=reservation_costs,
                    )
                )
            ...
            for event in aggregated_udi_events:
                if event.costs < udi_event_costs:
        ...
        # Create FlexOffer
        offered_values = best_udi_event.offered_power.loc[
            flex_request.start : (flex_request.end - flex_request.resolution)
        ]

        planned_flex = best_udi_event.planned_flex.loc[
            flex_request.start : (flex_request.end - flex_request.resolution)
        ]

        print("TA: Costs Best Udi event: {}\n".format(best_udi_event.costs))

        reservation_costs = best_udi_event.costs
    ...
    def step(self):
        print("################## NEW STEP ########################       TIME: {}".format(self.environment.now))
        print()
        ...
        self.environment.plan_board.store_message(timeperiod=self.environment.now, message=prognosis_request, keys=["TA", "MA"])
        ...
        table_snapshots(snapshots=self.store_table_steps,
        ...
        if "Not Cleared" in prognosis_decision_gate_1["Status"]:
            ...
            self.cleared_prognosis_negotiations.loc[self.environment.now, "Clearing Price"] = prognosis_decision_gate_1["Clearing price"]
            pass
        ...
        for ems in self.ems_agents:
            ...
            device_message = self.create_device_message(
                ...
                deviation_cost_curve=DeviationCostCurve(gradient=0, flow_unit_multiplier=self.environment.flow_unit_multiplier),
            )
        ...
        print("TA: Received requested flex values in kWh:{}\n".format(flex_request.requested_flex))
        print("TA: Received requested power values in kWh:{}\n".format(flex_request.commitment.constants.values))

        print("---------------------FLEX UDI EVENTS--------------------------      TIME: {}".format(self.environment.now))
        # Determine FlexOffer (could be multiple offers)
        flex_offer, flex_offer_udi_events = self.create_flex_offer(flex_request)
        # MA stores negotiated flext
        self.market_agent.get_flex_offer(flex_offer)
        self.environment.plan_board.store_message(timeperiod=self.environment.now, message=flex_offer, keys=["TA", "MA"])

        # Reservation values: TA = flex offer costs; MA = flex offer costs * Reservation factor
        # self.flexrequest_parameter["Reservation price"] = flex_offer.costs

        flex_offer_plus_deviation_costs = flex_offer.costs
        cost_only_flex = flex_offer.planned_flex.values.sum() * 1
        ...
        print("TA: Planned - Requested flex: {}\n".format(flex_offer.planned_flex - flex_request.requested_flex))
        ...
        self.environment.market_agent.flexrequest_parameter["Reservation price"] = sum(self.environment.market_agent.imbalance_market_costs.loc[self.environment.now:self.environment.now+self.prognosis_horizon])
        ...
        print("TA: Flex offer costs: {}".format(self.flexrequest_parameter["Reservation price"]))
        print("TA: Market agent reservation price {}".format(self.environment.market_agent.flexrequest_parameter["Reservation price"]))
        ...
        print("\nTA: Flex negotiation status: {}".format(flexrequest_decision_gate_1["Status"]))
        print("TA: Flex negotiation clearing price: {}\n".format(flexrequest_decision_gate_1["Clearing price"]))
        print("----------------------REALISED VALUES UPDATE--------------------------       TIME: {}".format(self.environment.now))
        ...
        if "NOT CLEARED" in flexrequest_decision_gate_1["Status"]:
            ...
            for ems in self.ems_agents:
                ...
                ems.realised_power_per_device.loc[self.environment.now,:] = ems.planned_power_per_device.loc[self.environment.now,:] - ems.planned_flex_per_device.loc[self.environment.now,:]
                ems.realised_costs_over_horizons[self.environment.now] = ems.prognosed_costs_over_horizons[self.environment.now]
                #ems.realised_flex_per_device.loc[self.environment.now,:] = ems.prognosed_flex_per_device.loc[self.environment.now,:]
                print("TA: Realised flex per device: {}\n".format(ems.realised_flex_per_device))
                print("TA: Realised power per device: {}\n".format(ems.realised_power_per_device))
            ...
            self.cleared_flex_negotiations.loc[self.environment.now, "Clearing Price"] = flexrequest_decision_gate_1["Clearing price"]
            for ems in self.ems_agents:
                ems.realised_power_per_device.loc[self.environment.now,:] = ems.planned_power_per_device.loc[self.environment.now,:]
                ems.realised_flex_per_device.loc[self.environment.now,:] = ems.planned_flex_per_device.loc[self.environment.now,:]
                ems.realised_costs_over_horizons[self.environment.now] = ems.planned_costs_over_horizons[self.environment.now]
            ...
            for row, val in enumerate(flex_offer.commitment.constants.loc[flex_offer.start : (flex_offer.end - flex_offer.resolution),]):
                if val != flex_offer.prognosis.commitment.constants[row]:
                    self.market_agent.commitments.loc[idx[row], "agreed_flex"] = self.market_agent.commitments.loc[idx[row], "negotiated_flex"]
            ...
            print("TA: MA AGREED flex: {} \n".format(self.market_agent.commitments["agreed_flex"]))
            print("TA: MA RECEIVED FLEX: {} \n".format(self.market_agent.commitments.loc[self.environment.now, "received_flex"]))
