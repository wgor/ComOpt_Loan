...
class MarketAgent(Agent):
    ...
    def __init__(
        ...
        columns = ["sold_power", "requested_flex", "negotiated_flex", "agreed_flex", "received_flex", "remaining_imbalances", "power_revenues", "flex_costs", "market_costs"]
        self.commitments = initialize_df(
            columns, environment.start, environment.end, environment.resolution
        )
        self.commitments.loc[:, "requested_flex"] = balancing_opportunities.loc[:, "Imbalance (in MW)"]
        self.commitments.loc[:, "remaining_imbalances"] = balancing_opportunities.loc[:, "Imbalance (in MW)"]
    ...
    def post_flex_request(self, prognosis: Prognosis) -> FlexRequest:
        ...
        if uniform(0,0.99) >= self.sticking_factor:
            ...
            remaining_commitment_opportunities = [nan if opportunity == 0 else opportunity for opportunity in remaining_commitment_opportunities]
            ...
            requested_values["requested_power"] = prognosis.commitment.constants.loc[
                flex_trade_window[0] : flex_trade_window[1] - self.environment.resolution] + remaining_commitment_opportunities

            print("MA: Already bought commitment: {}".format(already_bought_commitment))
            print("MA: Remaining commitment opportunities: {}\n".format(remaining_commitment_opportunities))
            print("MA: Requested Power values: {}\n".format(requested_values["requested_power"]))
            print("MA: Requested Flex values: {}\n".format(requested_values["requested_flex"]))
        ...
        return FlexRequest(
            ...
            deviation_cost_curve=DeviationCostCurve(
                gradient=(self.deviation_prices.loc[self.environment.now]*-1,self.deviation_prices.loc[self.environment.now]),
    ...
    def get_flex_offer(
        self, flex_offer: FlexOffer) -> FlexOffer:
        ...
        if flex_offer is not None:
            ...
            if isnull(self.commitments.loc[self.environment.now,"negotiated_flex"]) == True:
                for row, val in enumerate(flex_offer.commitment.constants.loc[flex_offer.start : (flex_offer.end - flex_offer.resolution),]):
                    if val != flex_offer.prognosis.commitment.constants[row]:
                        ...
                        for ems in self.environment.ems_agents:
                            total_ems_flexibility += ems.planned_flex_per_device.loc[idx[row],:].sum(axis=0)
                        self.commitments.loc[idx[row], "negotiated_flex"] = total_ems_flexibility
        # print("MA: Negotiated Flex: {} \n".format(self.commitments["negotiated_flex"]))

        return flex_offer

    def post_flex_order(
        self, flex_offer: FlexOffer) -> Optional[FlexOrder]:
        ...
        if flex_order is not None:
            ...
            for row, val in enumerate(flex_offer.commitment.constants.loc[flex_order.start : flex_order.start,]):
                if val != flex_offer.prognosis.commitment.constants[row]:
                    ...
                    self.commitments.loc[idx[row], "received_flex",] = total_ems_flexibility
                    self.commitments.loc[idx[row], "remaining_imbalances",] = self.balancing_opportunities.loc[idx[row],"Imbalance (in MW)",] - total_ems_flexibility

                    print("MA: Total EMS Flexibility: {}".format(total_ems_flexibility))
                    print("MA: Remaining Imblances: {}".format(self.commitments.loc[idx[row], "remaining_imbalances",]))
    ...
    def step(self):
        ...
        self.deviation_prices_realised.append(self.deviation_prices[self.environment.now])
