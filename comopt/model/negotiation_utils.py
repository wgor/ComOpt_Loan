...
def start_negotiation(
    ...
    for round in range(1, rounds_total + 1):
        ...
        if ma["Bid"] >= ta["Bid"]:
            ...
            q_table_df.loc[round_now, ta["Action"]] = update_q_table(
                action_table_df=action_table_df,
                q_table_df=q_table_df,
                q_parameter=q_parameter,
                action=ta["Action"],
                state_now=round_now,
            ...
            return {"Status": "Cleared", "Clearing price": ma["Bid"], "Negotiated issue":negotiation_issue}
    ...
    return {
        ...
        "TA last bid": ta["Bid"],
        "MA last bid": ma["Bid"],
        "Negotiated issue":negotiation_issue
    }
...
def update_q_table(
    ...
    updated_q_value = q_table_df.loc[state_now, action] + q_parameter["Alpha"] * (
        reward
        + q_parameter["Gamma"] * q_table_df.loc[state_next, :].max()
...
def table_snapshots(
    snapshots: int,
    step_now: int,
    timeperiod_now: DatetimeIndex,
    stored_q_tables: dict,
    stored_action_tables: dict,
    q_table_now: dict,
    action_table_now: dict,
):
