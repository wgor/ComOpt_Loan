...
def battery_solver(name,
    ...
    type:
        ...
        Determines wheter the base profile without requests or a flex request gets solved.
    mode:
    ...
    '''
    #TODO: Find efficient way to iterate over active ems in order to assign flex_request LpVariables(see file origin_solver).
    ...
    for time, ems in timeseries.index:
        ...
        if time == 1:
            milp += cap[(1,ems)] == parameter[ems]["Battery-SOC-Init"]-dis[(time, ems)]+char[(time, ems)]
        else:
